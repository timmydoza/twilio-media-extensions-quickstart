...
window.putMetadataAsync = (payload) => action('Timed metadata sent: ')(payload);
    ...
    Promise.resolve([
      {
        ...
        peerConnectionId: '',
        remoteAudioTrackStats: [{ trackSid: '1', audioLevel: 10 }],
...
class MockParticipant extends EventEmitter {
  ...
  get audioTracks() {
    const audioTrack = this.tracks.get('audio');
    if (audioTrack) {
      return new Map([['audio', audioTrack]]);
    } else {
      return new Map();
    }
  }
...
export function decorator(story, { args }) {
  for (let i = 1; i <= 15; i++) {
    ...
    if (mockParticipant) {
      const audioTrackPublication = mockParticipant.tracks.get('audio');
